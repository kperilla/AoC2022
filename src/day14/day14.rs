...
impl GridMap {
    ...
    fn drop_sand(&mut self, src_x: usize, is_infi: bool) -> Option<Point> {
        ...
        loop {
            ...
            let down_cand = Point{x: pos.x, y: pos.y + 1};
            let down_l_cand = Point{x: pos.x - 1, y: pos.y + 1};
            let down_r_cand = Point{x: pos.x + 1, y: pos.y + 1};
...
impl LineSegment {
    ...
    fn new(start: Point, stop: Point) -> Self {
        if start.x != stop.x && start.y != stop.y {
            ...
            Self{start, stop, orientation: LineOrientation::Vertical}
        } else {
            Self{start, stop, orientation: LineOrientation::Horizontal}
        }
    }
}
...
pub fn part1(input: String) {
    ...
    let mut map = GridMap{grid: HashMap::new(), min_x: None, max_x: 0, min_y: 0, max_y: 0};
    for shape in shapes {
        for seg in shape {
    ...
    let mut grains = 0;
    while let Some(_) = map.drop_sand(500, true) { grains += 1; }
    map.print_map();
...
pub fn part2(input: String){
    ...
    let ground_left = Point{x: 0, y: map.max_y + 2};
    let ground_right = Point{x: map.max_x * 2, y: map.max_y + 2};
    let ground = LineSegment{start: ground_left, stop: ground_right, orientation: LineOrientation::Horizontal};
    map.populate_ground_from_line(&ground, false);
    let mut grains = 0;
    while let Some(_) = map.drop_sand(500, false) { grains += 1; }
    map.print_map();
    println!("Grains: {}", grains);
}
